...
if vars_used:
    orig_var_sites = int(os.popen("grep -v '#' "+wd+"/pipeline/mutations/polymorphisms.vcf | wc -l").read())
    mv_var_sites = int(os.popen("grep -v '#' "+wd+"/pipeline/MV/all_chr_trio.downsampled.sorted.mark_dups.MV.polymorphisms.vcf | wc -l").read())
else:
    orig_var_sites = 0
    mv_var_sites = 0
orig_mut_sites = int(os.popen("grep -v '#' "+wd+"/pipeline/mutations/mutations.vcf | wc -l").read())
mv_mut_sites = int(os.popen("grep -v '#' "+wd+"/pipeline/MV/all_chr_trio.downsampled.sorted.mark_dups.MV.mutations.vcf | wc -l").read())

original_sites = orig_mut_sites + orig_var_sites
mv_sites = int(os.popen("grep -v '#' "+wd+"/pipeline/MV/all_chr_trio.downsampled.sorted.mark_dups.MV.vcf | wc -l").read())
...
for filter_name in list(input_file['filters'].keys()):
    ...
    for val in np.round(np.arange(float(min_value),float(max_value) + float(step) / 2,float(step)),2):
        total_sites = int(os.popen("grep -v '#' "+wd+"/pipeline/filters/"+filter_name+"/all_chr_trio.downsampled.sorted.mark_dups.MV."+filter_name+"."+str(val)+".vcf | wc -l").read())
        mut_sites = int(os.popen("grep -v '#' "+wd+"/pipeline/filters/"+filter_name+"/all_chr_trio.downsampled.sorted.mark_dups.MV."+filter_name+"."+str(val)+".mutations.vcf | wc -l").read())
        
        if vars_used:
            var_sites = int(os.popen("grep -v '#' "+wd+"/pipeline/filters/"+filter_name+"/all_chr_trio.downsampled.sorted.mark_dups.MV."+filter_name+"."+str(val)+".polymorphisms.vcf | wc -l").read())
